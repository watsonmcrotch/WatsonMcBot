...
class MissileHandler:
    def __init__(self, db_manager, send_companion_event):
        ...
        self.tts_output = str(BASE_DIR / 'sounds' / 'missiletts.mp3')
    ...
    async def process_missiletts_redeem(self, channel, username: str, story_text: str):
        try:
            ...
            if await self.generate_tts(story_text):
                async def play_missile_tts():
                    await play_audio_file_async(self.tts_output, 1.0)

                await self.tts_queue.add_tts(f"missile_{username}", play_missile_tts)
