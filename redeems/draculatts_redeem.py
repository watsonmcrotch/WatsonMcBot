...
class DraculaHandler:
    def __init__(self, db_manager, send_companion_event):
        ...
        self.tts_output = str(BASE_DIR / 'sounds' / 'draculatts.mp3')
        self.default_volume = 0.7

    def play_sound(self, sound_path: str, volume: float = None):
        if volume is None:
        ...
        try:
            ...
            adjusted_audio = audio.apply_gain(db_adjustment)
            
            temp_wav = f"{sound_path}_{time.time()}_temp.wav"
            adjusted_audio.export(temp_wav, format="wav")
            
            logging.info(f"Playing sound at {volume*100}% volume ({db_adjustment} dB adjustment)")
            ...
            def play_and_cleanup():
                try:
                    ...
                    try:
                        ...
                        logging.warning(f"Failed to remove temp file {temp_wav}: {e}")
                        pass
    ...
    async def process_draculatts_redeem(self, channel, username: str, story_text: str):
        try:
            ...
            if await self.generate_tts(story_text):
                async def play_dracula_tts():
                    await play_audio_file_async(self.tts_output, self.default_volume)

                await self.tts_queue.add_tts(f"dracula_{username}", play_dracula_tts)

            else:
                await channel.send(f"Sorry {display_name}, I couldn't generate the TTS...")
